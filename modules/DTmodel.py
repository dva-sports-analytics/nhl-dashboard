...
class DTClassifier():
	...
	def plot_heatmap(self):
	  ...
	  # If You are not training and prediction within the model
	  self.specificity = 0.13631338333283533
	  self.X_test_final = pd.read_csv("data/predictions.csv")
...
if __name__ == '__main__':
	print('Testing DTClassifier')
	df = pd.read_csv('../data/shots-2017-2020.csv')
	# DTmodel()
	DT = DTClassifier()
	DT.model_path = '../assets/decision_tree_clf_model.pkl'
	DT.hockey_rink_rev_filepath = '../assets/Half_ice_hockey_rink_rev.png'
	DT.data_filepath ='../data/shots-2017-2020.csv'
	DT.load_model()
	DT.predict()
	fig = DT.plot_heatmap()
	fig.show()
	# X_train, X_test, y_train, y_test = DT.preprocess_data(df=df)
	# print(X_train)
